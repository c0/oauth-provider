...
module OAuth
  module Controllers
    ...
    module ProviderController
      ...
      def request_token
        @token = current_app.create_request_token params
        if @token
      ...
      def revoke
        ...
        if @token
          @token.invalidate!
          flash[:notice] = "You've revoked the token for #{@token.app.name}"
        end
      ...
      def oauth2_token_authorization_code
        @verification_code =  @app.oauth2_verifiers.find_by_token params[:code]
      ...
      def oauth2_token_password
        ...
        @token = Oauth2Token.create :app=>@app, :user=>@user, :scope=>params[:scope]
        render :json=>@token
      end
      ...
      # autonomous authorization which creates a token for apps user
      def oauth2_token_client_credentials
        @token = Oauth2Token.create :app=>@app, :user=>@app.user, :scope=>params[:scope]
        render :json=>@token
      end
